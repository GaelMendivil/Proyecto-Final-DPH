...
public async Task<IActionResult> SearchPokemon(string term)
{
    ...
    foreach (var p in filtered)
    {
        ...
        string pokeUrl = p["url"].ToString();   
        string id = pokeUrl.TrimEnd('/').Split('/').Last();

        string spriteApiUrl = $"https://pokeapi.co/api/v2/pokemon/{id}/";
        var pokeResponse = await _httpClient.GetStringAsync(spriteApiUrl);
        var pokeJson = JObject.Parse(pokeResponse);

        string retroSprite =
            pokeJson["sprites"]["versions"]["generation-v"]["black-white"]["front_default"]
            ?.ToString() ?? "";

        results.Add(new
        {
            name = name,
            imageUrl = retroSprite
        });
    }
